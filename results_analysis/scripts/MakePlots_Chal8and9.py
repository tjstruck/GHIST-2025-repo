...
for chal_deets in [('multi_sweep_bgs', 'chal9'), ('multi_sweep', 'chal8')]:
    chal, chal_num = chal_deets
    # In a BED, the start is the equivalent of 0-based position for SNPs
    sweeps = [int(ele.split('\t')[1])+1 for ele in open(f"data/groundtruth/GHIST_2025_{chal}_final_goldstandard.bed").readlines()]
    ...
    # Path to VCF
    if "_bgs" in chal:
        vcf_fi = "data/vcf/GHIST_2025_multisweep.growth_bg.15.final.vcf.gz"
    else:
